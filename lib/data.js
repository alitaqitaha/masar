...
export async function fetchAttendanceRecordsPage(instituteId, teacherId, offset = 0, pageSize = 20, groupId = null) {
  let query = supabase
...
export async function fetchGradeRecordsPage(instituteId, teacherId, offset = 0, pageSize = 20, groupId = null) {
  let query = supabase
    .from("grade_records")
    .select(GRADES_COLS)
    .eq("institute_id", instituteId)
...
export async function addStudent(instituteId, { name, phone, parentPhone, photo, username, password, enrollments }, serial) {
  ...
  const json = await res.json();
  if (!res.ok) {
    const err = new Error(json.error || "create-student failed");
    throw err;
  }
  return serial;
}
...
export async function setAttendanceExcused(attendanceRecordId, studentId, excused) {
  await mutate("setAttendanceExcused", { attendanceRecordId, studentId, excused });
}
...
export async function loginTeacherByCode(code) {
  ...
  return json; // { id, name, subjectId, instituteId }
}
